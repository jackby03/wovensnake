...
### Added
- **`scripts/uninstall.sh`**: standalone uninstaller script that works even if the binary is broken or not in PATH. Finds the binary in common locations, removes it, clears `~/.wovensnake`, and strips PATH entries from shell rc files.

### Fixed
- **`woven self-uninstall`**: PATH cleanup now correctly matches the `# WovenSnake` comment block and `.wovensnake/bin` entries written by `install.sh`.

## [0.3.3] - 2026-02-20

### Added
- **`woven self-uninstall`**: new command that removes the binary, clears `~/.wovensnake`, and strips the PATH entry from `~/.bashrc`, `~/.zshrc`, and `~/.profile`. Pass `--yes` to skip the confirmation prompt.
